...
import { ChevronLeft, ChevronRight, Save, Clock, User, Calendar } from 'lucide-react';
...
export const MobileSchichtplanungView = () => {
  ...
  const { userProfile, isAbteilungsleiter } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      const { data, error } = await supabase
        .from('employees')
        .select('id, vorname, nachname, personalnummer, abteilung')
  ...
  const handleShiftClick = (employee: Employee, date: Date) => {
  ...
  const handleSaveShift = async () => {
    ...
    try {
      ...
      if (data) {
        setShifts(prev => ({ ...prev, [key]: data as Shift }));
      }
  ...
  const getShiftDisplay = (employee: Employee, date: Date) => {
    const key = `${employee.id}_${format(date, 'yyyy-MM-dd')}`;
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {weekDays.map((day) => (
                      <div
                        ...
                        onClick={() => handleShiftClick(employee, day)}
                      >
